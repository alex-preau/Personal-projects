...
class PF(object):
    ...
    def prediction(self, u, deltat):
        ...
        """
            ...
            deltat :  Step size
        """
        #print(self.Alpha[0] * u[0] ** 2 + self.Alpha[1] * u[1] ** 2)
        v_noise = np.random.normal(0, self.Alpha[0] * u[0] ** 2 + self.Alpha[1] * u[1] ** 2,size=self.numParticles)
        w_noise = np.random.normal(0, self.Alpha[2] * u[0] ** 2 + self.Alpha[3] * u[1] ** 2, size=self.numParticles)
        gamma_noise = np.random.normal(0, self.Alpha[2] * u[0] ** 2 + self.Alpha[3] * u[1] ** 2,size=self.numParticles)
        i = 0
        ...
        #    print("i == ",i)
            ...
            #print(w)
            gamma = gamma_noise[i]
            pos, weight = self.getParticle(i)
            old_x,old_y,old_theta = pos
            new_x = old_x + v/w * (np.sin(self.angleWrap(old_theta + w * deltat)) - np.sin(old_theta))
            new_y = old_y + v/w * (np.cos(old_theta) - np.cos(self.angleWrap(old_theta + w * deltat)))
            new_theta = self.angleWrap(old_theta + w * deltat + gamma * deltat)
            #print(new_x)
            #print(new_y)
            #print(new_theta)
            if not self.gridmap.inCollision(new_x,new_y):
                self.particles[0, i] = new_x
                self.particles[1, i] = new_y
                self.particles[2, i] = new_theta
            i += 1
            # else:
            #     #is collision, resample
            #     print("resampling, i == ",i,'num particles is ',self.numParticles)
            #     v_noise[i] = np.random.normal(0, self.Alpha[0] * u[0] ** 2 + self.Alpha[1] * u[1] ** 2)
            #     w_noise[i] = np.random.normal(0, self.Alpha[2] * u[0] ** 2 + self.Alpha[3] * u[1] ** 2)
            #     gamma_noise[i] = np.random.normal(0, self.Alpha[2] * u[0] ** 2 + self.Alpha[3] * u[1] ** 2)
    ...
    def resample(self):
        ...
        #print(weights)
    #    print(weights[0])
        indecies = np.random.choice(self.numParticles,self.numParticles,p=weights[0])

        new_particles = np.empty([3, self.numParticles])
        j = 0
        for i in indecies:
            if add_noise:
                #adding noise gives much better results, although it runs slower
                new_particles[:,j] = self.particles[:,i] + np.random.normal(0, sigma,3)
            else:
    ...
    def run(self, U, Ranges, deltat, X0, XGT, filename):
        ...
        for k in range(U.shape[1]):
            ...
            #print(self.weights[0,:])
        #    print("weight sum is ",np.sum(self.weights[0,:]))
            if k % 10 == 0:
                self.update(ranges)
            #if np.sum(self.weights[0,:]) < .4 :#or k % 30 == 0:#k % 7 == 0  :
                print("resampling")
                self.resample()
        #        self.update(ranges)
