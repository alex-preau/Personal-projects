...
class Laser(object):
    ...
    def __init__(self, numBeams=9, sparsity=1):
    ...
    def scanProbability(self, z, x, gridmap):
        ...
        qs = []
        #x = (x[:,None])
        z_star_all= self.rayTracing(x[0,:],x[1,:],x[2,:],self.Angles,gridmap)
    #    print(z_star_all)

        for i in range(len(x[0])):
            q = 1
            for k in range(self.numBeams):
            #    print(self.numBeams)
            #    print(k)
                #z_star= self.rayTracing(x[0],x[1],x[2],self.Angles[k],gridmap)
                #print(z_star_all)
            #    print(z_star_all)
                z_range  = z_star_all[0][i][k]
                #print(z_range)
            #    a = 1/0
                #print(z_range)
                #print("z star is ",z_star)
                #print("z is ",z)



                p_hit = self.normal.pdf(z_range - z[k]) #because it is a 0-mean gaussian

                if z[k] > z_range or z[k] < 0:
                    p_short = 0
                else:
                    p_short = self.exponential.pdf(z[k])
                ...
                if z[k] > 19.9 and z[k] < 20.1:
                    #unsure if this is how I should be doing it? but how bad can it be
                    p_max = 1 #idk if this should be 1 either
                else:
                    p_max = 0

                p =  self.pHit * p_hit +  self.pShort * p_short+  self.pMax * p_max+   self.pRand * p_rand
                q = q * p
